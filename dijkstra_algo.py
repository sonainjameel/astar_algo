...
class Dijkstra:
    ...
    def search(self):
        ...
        self.distance[self.start] = self.costToCome[self.start]
        ...
        while queue:
            ...
            for leftRPM, rightRPM in [(0, self.wheelRPM[0]), (self.wheelRPM[0], 0), (self.wheelRPM[1], self.wheelRPM[1])]:
                ...
                if flag:
                    ...
                    if self.costToCome.get(newState, float('inf')) > newCostToCome:
                        self.costToCome[newState] = newCostToCome
                        self.distance[newState] = newCostToCome
                        self.path[newState] = (current, cost)
                        heappush(queue, (newCostToCome, newState))
...
class PathPlannerNode(Node):
    ...
    def create_plan_cb(self, request, response):
        ...
        response.path = self.create_dijkstra_plan(start_pose, goal_pose, time_now, global_costmap)
        return response
    ...
    def create_dijkstra_plan(self, start, goal, time_now, costmap):
        path = Path()
        ...
        dijkstra = Dijkstra(start_coords, goal_coords, (100, 50), clearance=50, costmap=costmap)

        if not dijkstra.IsValid(start_coords[0], start_coords[1]) or dijkstra.IsObstacle(start_coords[0], start_coords[1]):
            self.get_logger().error("Start point is invalid or inside an obstacle.")
            return path
        if not dijkstra.IsValid(goal_coords[0], goal_coords[1]) or dijkstra.IsObstacle(goal_coords[0], goal_coords[1]):
            self.get_logger().error("Goal point is invalid or inside an obstacle.")
            return path

        explored_states, backtrack_states = dijkstra.search()
        if not backtrack_states:
            self.get_logger().error("No valid path found by Dijkstra.")
            return path
